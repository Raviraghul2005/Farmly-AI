...
export class SchemeService {
  ...
  private static calculateEligibilityMatch(user: IUser, scheme: IScheme): number {
    let score = 0;
    let maxScore = 0;
    ...
    if (scheme.eligibility.crops && scheme.eligibility.crops.length > 0) {
      maxScore += 40;
      const schemeCrops = scheme.eligibility.crops;
      const matchingCrops = userCrops.filter((crop: string) =>
        schemeCrops.includes(crop.toLowerCase())
      );
